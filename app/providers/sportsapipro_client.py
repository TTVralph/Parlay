...
class SportsAPIProClient:
    def __init__(
        self, api_key: str, base_url: str = _BASE_URL, timeout_seconds: float = 15.0
    ) -> None:
        if not api_key:
    ...
    def _request_json(self, path: str, params: dict[str, Any] | None = None) -> Any:
        ...
        logger.debug(
            "SportsAPI Pro request path=%s params=%s",
            path,
        ...
        try:
            ...
            logger.warning("SportsAPI Pro timeout path=%s", path)
            raise SportsAPIProError(
                "SportsAPI Pro request timed out",
                status_code=504,
                code="sportsapipro_timeout",
            ) from exc
        except httpx.RequestError as exc:
            logger.exception("SportsAPI Pro request failed path=%s error=%s", path, exc)
            raise SportsAPIProError(
                "SportsAPI Pro request failed", code="sportsapipro_request_error"
            ) from exc

        if response.status_code in (401, 403):
            logger.warning(
                "SportsAPI Pro auth failure path=%s status=%s",
                path,
        ...
        try:
            ...
            raise SportsAPIProError(
                "SportsAPI Pro returned invalid JSON", code="sportsapipro_invalid_json"
            ) from exc
