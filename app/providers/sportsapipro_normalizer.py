...
def normalize_game(row: dict[str, Any]) -> dict[str, Any]:
    ...
    competition = (
        row.get("competition") if isinstance(row.get("competition"), dict) else None
    )
    ...
    return {
        ...
        "competitionId": (
            str(competition.get("id"))
            if competition and competition.get("id") is not None
            else None
        ),
        "competitionName": competition.get("name") if competition else None,
        "homeTeam": (
            (home_team or {}).get("name")
            if isinstance(home_team, dict)
            else row.get("homeTeamName")
        ),
        "awayTeam": (
            (away_team or {}).get("name")
            if isinstance(away_team, dict)
            else row.get("awayTeamName")
        ),
        "homeTeamId": (
            str((home_team or {}).get("id"))
            if isinstance(home_team, dict) and (home_team or {}).get("id") is not None
            else None
        ),
        "awayTeamId": (
            str((away_team or {}).get("id"))
            if isinstance(away_team, dict) and (away_team or {}).get("id") is not None
            else None
        ),
        "status": status,
...
def normalize_athlete_game_logs(athlete_id: str, payload: Any) -> list[dict[str, Any]]:
    ...
    for row in rows:
        if not isinstance(row, dict):
            continue
        opponent = (
            row.get("opponent") if isinstance(row.get("opponent"), dict) else None
        )
        competition = (
            row.get("competition") if isinstance(row.get("competition"), dict) else None
        )
        ...
        output.append(
            {
                ...
                "gameId": (
                    str(row.get("gameId")) if row.get("gameId") is not None else None
                ),
                "date": row.get("date") or row.get("gameDate") or row.get("startTime"),
                "opponentId": (
                    str(opponent.get("id"))
                    if opponent and opponent.get("id") is not None
                    else None
                ),
...
def _normalized_game(event: dict[str, Any]) -> dict[str, Any] | None:
    ...
    return {
        "id": game_id,
        "competitionId": _extract_id(_extract_from_any(event, ("competitionId",)))
        or _extract_id(competition),
        "competitionName": _extract_name(_extract_from_any(event, ("competitionName",)))
        or _extract_name(competition),
        ...
        "startTime": _as_string(
            _extract_from_any(event, ("startTime", "startDate", "date", "scheduledAt"))
        ),
    }
...
def normalize_games_payload(payload: Any) -> list[dict[str, Any]]:
    ...
    if isinstance(payload, list):
        ...
        possible_rows = _extract_from_any(
            payload, ("games", "results", "data", "events")
        )
        rows = possible_rows if isinstance(possible_rows, list) else [payload]
...
def normalize_athlete_games_payload(
    athlete_id: str, payload: Any
) -> list[dict[str, Any]]:
    rows: list[Any] = []
    headers: Any = []
    if isinstance(payload, dict):
        ...
        headers = (
            _extract_from_any(payload, ("headers", "statHeaders", "columns")) or []
        )
        ...
        output.append(
            {
                ...
                "gameId": game_id,
                "date": _as_string(
                    _extract_row_value(row, "date", "gameDate", "startTime")
                ),
                "opponentId": _extract_id(
                    _extract_row_value(row, "opponentId", "opponentTeamId")
                )
                or _extract_id(opponent),
                "opponentName": _extract_name(_extract_row_value(row, "opponentName"))
                or _extract_name(opponent),
                "competitionId": _extract_id(_extract_row_value(row, "competitionId"))
                or _extract_id(competition),
                "competitionName": _extract_name(
                    _extract_row_value(row, "competitionName")
                )
                or _extract_name(competition),
                "stats": stats,
            }
